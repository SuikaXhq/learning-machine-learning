...
class dPCA(BaseEstimator):
    ...
    [ 0.99244...  0.00755...]
    """
    def __init__(self, labels=None, join=None, n_components=10, regularizer=None, copy=True, n_iter=0,cross=False):
        ...
        self.n_iter = n_iter
        self.start = self.end = self.total_time = self.regularize_time = 0
    ...
    def _optimize_regularization(self,X,trialX,center=True,lams='auto'):
        ...
        # print('stack',np.dstack([scores[key] for key in list(scores.keys())]))
        # print('totalscore',totalscore)
        # print('shape',totalscore.shape)
        ...
        print('betst regularize:',self.regularizer)
        ...
        temp_2 = time.time()
        self.regularize_time = temp_2 - temp_1
    ...
    def crossval_score(self,lams,X,trialX,mean=True):
        ...
        scores = np.zeros(len(lams)) if mean else {key : np.zeros(len(lams)) for key in list(self.marginalizations.keys())}
        # print('in crossval:', type(trialX))
        # compute number of samples in each condition
        XmXs = self._marginalize(X)
        for k, lam in enumerate(lams):
            # fit dpca model
            self.regularizer = lam
            self._fit(X,mXs=XmXs,optimize=False)
            # compute crossvalidation score
            if mean:
                scores[k] = self._score(X,XmXs)
            else:
                tmp = self._score(X,XmXs,mean=False)
                for key in list(self.marginalizations.keys()):
                    scores[key][k] = tmp[key]
    ...
    def _score(self,X,mXs,mean=True):
        ...
        # print(np.dot(self.P['s'],np.dot(self.D['s'].T,X)).shape,'P',self.P['s'].shape)
    ...
    def _randomized_dpca(self,X,mXs,pinvX=None):
        ...
        for key in list(mXs.keys()):
            ...
            # print('D.shape',D[key].shape,'P.shape',P[key].shape)
        return P, D
    ...
    def transform(self, X, marginalization=None):
        ...
        def marginal_variances(marginal):
            ...
            # Xmarg = self.Xmargs[marginal].reshape(X.shape[0],-1)
            # print('Xmarg.shape',Xmarg.shape)
        ...
        self.end = time.time()
        self.total_time = self.end - self.start - self.regularize_time
