...
'''
    ...
    def transform(self, X, marginalization=None):
        ...
        def marginal_variances(marginal):
            ...
            D, F, Xr= self.D[marginal], self.F[marginal], X.reshape((X.shape[0],-1))
            Xmarg = self.Xmargs[marginal].reshape(X.shape[0],-1)
            return [(np.sum(Xmarg**2)-np.sum((Xmarg-np.outer(F[:,k],np.dot(D[:,k],Xmarg)))**2)) / total_variance for k in range(D.shape[1])]
    ...
    def inverse_transform(self, X, marginalization):
        X = self._zero_mean(X)
        X_transformed = np.dot(self.F[marginalization], X.reshape((X.shape[0], -1))).reshape(
            (self.F[marginalization].shape[0],) + X.shape[1:])
